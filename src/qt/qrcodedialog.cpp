...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non HDC unit input in lnReqAmount, we generate the URI with HDC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::HDC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
